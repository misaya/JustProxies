    ...
    : HttpBaseInterceptor(logger)
{
    public override async Task Server_OnRequestReceived(IHttpProxyServer server, HttpRequestReceivedEventArgs e)
    {
        ...
        if (package != null)
        {
            logger.LogInformation($"来自{e.HttpContext.RemoteEndPoint}的请求{e.HttpContext.Request}被截获处理");
            await package.ApplyRuleAction(e.HttpContext);
        }
        else
        {
            logger.LogInformation($"来自{e.HttpContext.RemoteEndPoint}的请求{e.HttpContext.Request}未被截获处理");
        }
    }
